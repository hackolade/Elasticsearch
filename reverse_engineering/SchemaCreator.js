...
module.exports = {
	...
	getSchema(elasticMapping, sample) {
		...
		schema.properties._source.properties = this.getFields(
	...
	getField(fieldData, sample, mapping) {
		...
		if (['geo-shape', 'geo-point'].includes(schema.type)) {
	...
	getCopyToPath(copyToValue, mapping) {
		...
		const result = copyTo.reduce((result, propertyName) => {
			return [...result, ...findPropertiesInMapping(propertyName, mapping)];
		}, []);
	...
	const getPaths = (propertyName, properties, path) => {
		return Object.keys(properties).reduce((result, name) => {
			...
			if (property.properties) {
				result = [...result, ...getPaths(propertyName, property.properties, [...path, name])];
			}

			if (propertyName === name) {
				result = [...result, [...path, name].join('.')];
			}
